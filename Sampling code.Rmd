...
#Adding in Nialls fake income data code
# Fake income data
...
#sampling using stratifying function
...
# Summary stats of actual frequiences of each strata
...
```{r}
# Creating sample diagnostic function to find average absolute percent difference in income between estimated income using the sample and the true income

sample_diagnostic_func<-function(loop.n){

#setting up vectors to store percent differences when stratified by either region or size
perc.diff.region <- rep(0,12)
perc.diff.size <- rep(0,5)

for (i in 1:loop.n){
  sample.data <- Stratified.sample.func(charity.data,1,0.8,0.45,0.15,120)
...
region.diag<-data.frame("region"= c("North East","North West","Yorkshire & the Hum","East Midlands","West Midlands","East of England","London","South East","South West","Wales","Scotland","Unkown"),
...
sample.diagnostics <- sample_diagnostic_func(1000)
```
...
```{r}
#Creating simple graphs to display percent of strata sampled

ggplot(data=est.region.income,aes(x=region,y=perc.sample,fill=region))+
  geom_bar(stat = "identity")+
  geom_text(aes(label = paste0(round(perc.sample,2),"%"," (N=",sample.region.n,")")), vjust = 1.5, colour = "white")+
  ylab("Percent of strata in sample")+
  xlab("Region")+
  ylim(0,100)



ggplot(data=est.size.income,aes(x=size,y=perc.sample,fill=size))+
  geom_bar(stat = "identity")+
  geom_text(aes(label = paste0(round(perc.sample,2),"%"," (N=",sample.size.n,")")), vjust = 1.5, colour = "white")+
  ylab("Percent of strata in sample")+
  xlab("Size")+
  ylim(0,100)
```
