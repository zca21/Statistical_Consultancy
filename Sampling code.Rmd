...
```{r}
#Using sample to estimate the total income by using weights
...
sample.stats <- sample.dataset%>%
  group_by(size,region)%>%
  summarise(total.sample=sum(Income),number.sample=n())

pop.stats <- charity.data%>%
  group_by(size,region)%>%
  summarise(total.pop=sum(Income),number.pop=n())

est.strata <- merge(sample.stats,pop.stats,by=c("size","region"))%>%
  mutate(total.est = number.pop/number.sample*total.sample,
         perc.difference = abs(1-(total.est/total.pop)))
#reporting differences for each strata
est.strata %>%
  kableExtra::kable(digits=3)%>%
  kableExtra::kable_classic()

#repeating but grouping by region
est.strata%>%
  group_by(region)%>%
  summarise(est.region=sum(total.est),
            actual.region=sum(total.pop),
            perc.difference = abs(1-(est.region/actual.region)))%>%
  kableExtra::kable(digits=3)%>%
  kableExtra::kable_classic()
...
est.strata%>%
  group_by(size)%>%
  summarise(est.size=sum(total.est),
            actual.size=sum(total.pop),
            perc.difference = abs(1-(est.size/actual.size)))%>%
  kableExtra::kable(digits=3)%>%
  kableExtra::kable_classic()
```
